...
def intent_planning_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        plan = intent_agent.analyze(state["query"])
...
def retrieval_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if decision and decision.tool == "web_search":
            ...
            logger.info("🔍 Using ToolAgent for web search...")
            query = decision.search_query or state["query"]
            ...
            logger.info("📖 Using RetrieverAgent for internal documents...")
            
            docs = retriever_agent.retrieve(state["query"], top_k=5)
